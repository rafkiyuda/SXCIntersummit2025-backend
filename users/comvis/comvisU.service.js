...
async function getAnnouncement(userId) {
  const division = 6; // Comvis division
...
async function sendEmailComvis(userId, link, qrcode) {
  const user = await prisma.user.findUnique({ where: { id: userId } });
  const html = await composeEmail({
    title: "Welcome to Comvis", // Judul untuk <title> tag dan header email
    bodyBlocks: [
      { type: "paragraph", text: `Dear ${user.name},` },
      {
        type: "paragraph",
        text: "Thank you for registering for Comvis 2025, a two-day offline career exploration event across Banking, Consulting, FMCG, and StartUp industries.",
      },
      {
        type: "paragraph",
      ...
      {
        type: "paragraph",
        text: "We’re excited to see you at Comvis 2025 and can’t wait to start this journey together ",
      },
  ...
  return await sendEmail(
    user.email,
    "[Confirmation] – Successful Registration of StudentsxCEOs InterSummit 2025 Comvis",
    html
  );
}

async function testEmail(userId) {
  const user = await prisma.user.findUnique({ where: { id: userId } });
  const html = await composeEmail({
    title: "Welcome to Comvis",
    bodyBlocks: [
      { type: "paragraph", text: `Dear ${user.name},` },
      {
        type: "paragraph",
